...
export default function Home() {
  ...
  useEffect(() => {
    setMounted(true);
    // Typing animation for title (halus, satu per satu, tidak lompat)
    let i = 0;
    const animate = () => {
      setTitleAnim(titleText.slice(0, i) + "•".repeat(titleText.length - i));
      if (i < titleText.length) {
        i++;
        setTimeout(animate, 300); // 250ms per huruf, bisa diubah sesuai selera
      }
    };
    animate();
  }, []);
  ...
  return (
    <div className="min-h-screen flex bg-gradient-to-r from-[#0B0B0B] via-[#181A1B] to-[#6B21A8]">
      {/* Left Panel */}
      <div
